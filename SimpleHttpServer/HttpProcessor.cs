...
//                Console.WriteLine("{0} {1}",response.StatusCode,request.Url);
		...
		private HttpRequest GetRequest(Stream inputStream, Stream outputStream) {
			...
			Console.WriteLine(System.Uri.UnescapeDataString(request));
...
//					throw new Exception("invalid http header line: " + line);
			...
			var GET = matches.Cast<Match>().ToDictionary(m => Uri.UnescapeDataString(m.Groups[2].Value),m => Uri.UnescapeDataString(m.Groups[3].Value.Replace("+", " ")));
